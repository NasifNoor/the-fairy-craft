...
export function CustomFurnitureSection({ data }: CustomFurnitureSectionProps) {
  return (
    <section id="custom-design" className="border-y border-[var(--color-border)] bg-[var(--color-surface)] py-20 sm:py-24">
      <Container className="space-y-8">
        <div className="max-w-2xl space-y-3">
          ...
          <p className="text-lg font-medium text-[var(--color-text)]">{data.subtitle}</p>
          <p className="text-base leading-8 text-[var(--color-text-muted)]">{data.description}</p>
        </div>

        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {data.steps.map((step, index) => (
            <Card key={step.title} padding="lg" className="space-y-3">
              <p className="text-sm font-semibold text-[var(--color-primary-700)]">0{index + 1}</p>
              <h3 className="text-lg font-semibold text-[var(--color-text)]">{step.title}</h3>
              <p className="text-sm leading-7 text-[var(--color-text-muted)]">{step.description}</p>
            </Card>
          ))}
        </div>
